...
export async function getUserPurchasedCourses(userId: string): Promise<{ course_id: string; courses: Course }[]> {
  ...
  if (error) throw error
  // Ensure each 'courses' is a single Course object, not an array
  return (data as any[]).map(row => ({
    course_id: row.course_id,
    courses: Array.isArray(row.courses) ? row.courses[0] : row.courses
...
export async function createSection(section: Omit<Section, 'id' | 'created_at' | 'updated_at'>): Promise<Section> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('sections')
    .insert([section])
...
// UserResource operations
export async function setUserResourceCompleted(userId: string, resourceId: string, completed: boolean): Promise<UserResource> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('user_resources')
    .upsert([{ user_id: userId, resource_id: resourceId, completed }], { onConflict: 'user_id,resource_id' })
    .select()
    .single()
  if (error) throw error
  return data
...
export async function getUserResource(userId: string, resourceId: string): Promise<UserResource | null> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('user_resources')
    .select('*')
